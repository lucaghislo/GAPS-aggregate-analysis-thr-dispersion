% Threshold dispersion analysis for each channel and ASIC

%% Global optimisation
clear all; clc;

% global vars
main_folder = '/home/lucaghislotti/Downloads/analisi_FEB_thr/Analisi aggregate_ASIC/';
...
for iasic=range
    ...
    if (exist(['ASIC_Test/ASIC_',number,'/1/analysis_matlab/ThresholdScan/fitParameters.dat']) && exist(['ASIC_Test/ASIC_',number,'/1/analysis_matlab/TransferFunction/Low_Energy_Gain.dat']))
        % threshold scan fit data
        data = importdata(['ASIC_Test/ASIC_',number,'/1/analysis_matlab/ThresholdScan/fitParameters.dat']);
        temp = data.data(:,4:5);
        % tf low gain (high charge section) data
        data2 = importdata(['ASIC_Test/ASIC_',number,'/1/analysis_matlab/TransferFunction/Low_Energy_Gain.dat']);
        for i=0:31
