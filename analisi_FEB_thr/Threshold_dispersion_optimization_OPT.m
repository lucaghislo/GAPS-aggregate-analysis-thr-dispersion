...
% Change to 'full' in Threshold_dispersions_ASIC.m
...
% ASICs means per tau
% init
asics_tau_means_kev = NaN(length(gbl_thr_range), 1); % length(gbl_thr_range)
asics_all_channels = [];
corrections_ASIC = zeros(length(gbl_thr_range), 2); % length(gbl_thr_range)
count_opt_arr = zeros(length(gbl_thr_range), 1); % length(gbl_thr_range)
corrections_ASIC = zeros(N, 2); %% N to change per classA to full
...
for iasic = gbl_thr_range
    ...
    Thr_no_out = Thr_best(iasic+1).mat(~isoutlier(Thr_best(iasic+1).mat(:, 7)),7); % Thr_best is FTHR
    ...
    asics_tau_means_kev = asics_tau_means_kev;
...
for iasic=1:length(gbl_thr_range)
