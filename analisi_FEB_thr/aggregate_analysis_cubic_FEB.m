...
for j=0:7
    ...
    caxis([0.85,1.15])
    set(gca,'FontSize',8);
    title(sprintf('Cubic gain (in x=0) for each channel in each FEB [ADCu / DAC\\_inju]. Peaking time #%i',j));
    file_out = sprintf([main_folder, 'Cubic_gain_tau%i.pdf'],j);
    exportgraphics(gcf,file_out,'ContentType','vector');
    file_out_fig = sprintf([main_folder, 'Cubic_gain_tau%i.fig'],j);
    savefig(file_out_fig);
    close(gcf);
